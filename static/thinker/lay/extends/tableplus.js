...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'excel', 'util'], function(exports){
    "use strict";
    ...
    Class.prototype.renderToolbar = function(){
        ...
        }, layui.conf.tableToolbar.layout || {}), iconElem = [];
    ...
    Class.prototype.pullData = function(curr){
        ...
        if(options.url){ //Ajax请求
            ...
            //判断是否存在thinker的请求，存在就直接使用
            var parseOptions = layui.view._parseRequest ? layui.view._parseRequest : function(options){
                return options;
            };
    ...
    Class.prototype.events = function(){
        ...
        that.layTool.on('click', '*[lay-event]', function(e){
            ...
            switch(events){
                        ...
                        openPanel({
                            list: function(){
                                return [
                                    ...
                                    ,'<li data-type="xlsx">导出到 Xlsx 文件</li>'
                                ].join('')
                            }()
                ...
                case 'LAYTABLE_PRINT': //打印
                    ...
                    break;
                default:
                    if(layui.conf.tableToolbar.event){
                        layui.conf.tableToolbar.event[events] &&
                        layui.conf.tableToolbar.event[events](that, othis);
                    }
            }
        ...
        that.layBody.on('click', 'td', function(e){
            var othis = $(this)
                ...
                ,parentKey = othis.data("key").split("-");
    ...
    table.exportFile = function(id, data, type){
        ...
        var dataTitle = [], dataMain = [], dataTotal = [], exportData = [];
        ...
        if(config.beforeExport){
            config.beforeExport(dataTitle, dataMain, dataTotal);
        }else{
            exportData.push(dataTitle);
            for(var i in dataMain){
                exportData.push(dataMain[i]);
            }
            exportData.push(dataTotal);
            layui.excel.exportExcel({
                sheet1: exportData
            }, (config.title || 'table_'+ (config.index || '')) + '.' + type, type);
        }
        // data = data || table.clearCacheKey(table.cache[id]);
        // type = type || 'csv';
        //
