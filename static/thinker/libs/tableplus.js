...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
    ...
    var $ = layui.$
        ...
        ,table = {
            ...
            ,cache: {} //数据缓存
            ,index: layui.tableplus ? (layui.tableplus.index + 10000) : 0
        ...
        //字符常量
        ,MOD_NAME = 'tableplus', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    }, (layui.conf ? layui.conf.table.config : {}));
    ...
    Class.prototype.render = function(){
        ...
        //充值是否渲染drag
        that.config.renderedTable = false;
        ...
        //判断是否需要重新渲染table
        if(layui.thinkerTable) {
            options.cols = layui.thinkerTable.renderCols(options);
        }
        ...
        options.response = $.extend({
            ...
            ,countName: 'count'
        }, (layui.conf ? layui.conf.response : {}), options.response);
        ...
        if(layui.thinkerTable) {
            //单个table拖拽只显示一次
            if(that.config.thinkerDrag && !that.config.renderedTable) {
                that.config.renderedTable = true;
                layui.thinkerTable.drag(that.config);
            }
        }
    };
    ...
    Class.prototype.renderToolbar = function(){
        ...
        var layout = $.extend({
            ...
            ,clearcols: {
                title: '清空列表调整'
                ,layEvent: 'LAYTABLE_CLEARCOLS'
                ,icon: 'layui-icon-fonts-clear'
            }
        }, (layui.conf ? layui.conf.table.toolbar.icons : {})), iconElem = [];

        if(typeof options.defaultToolbar === 'object'){
            if(options.thinkerCache) {
                if(options.defaultToolbar.indexOf("clearcols") == -1) {
                    options.defaultToolbar.push("clearcols");
                }
            }
    ...
    Class.prototype.pullData = function(curr){
        ...
        if(options.url){ //Ajax请求
            ...
            $.ajax({
                ...
                ,success: function(res, textStatus, xh){
                    //如果有数据解析的回调，则获得其返回的数据
                    if(typeof options.parseData === 'function'){
                        res = options.parseData(res, textStatus, xh) || res;
                    }
                    //检查数据格式是否符合规范
    ...
    Class.prototype.loading = function(hide){
        ...
        if(options.loading){
            if(hide){
                ...
                delete that.layInit;
                that.layBox.find(".layui-table-init").remove();
            } else {
                that.layInit = layui.$("<div class='thinker-loading-mask'><div class='thinker-loading'></div></div>");
    ...
    Class.prototype.events = function(){
        ...
        that.layTool.on('click', '*[lay-event]', function(e){
            ...
            switch(events){
                case 'LAYTABLE_REFRESH': //刷新
                    that.reload();
                    break;
                case 'LAYTABLE_CLEARCOLS': //刷新
                    if(layui.thinkerTable) {
                        layui.layer.confirm("是否确认清除列表调整<br/>(包括显示隐藏列以及顺序调整)?", function(index){
                            layui.layer.close(index);
                            layui.thinkerTable.clearSaveCols(options);
                        });
                    } else {
                        layui.layer.alert("当前未引入thinkerTable,无法清除");
                    }
                    break;
                    ...
                    openPanel({
                        ...
                        ,done: function(){
                            form.on('checkbox(LAY_TABLE_TOOL_COLS)', function(obj){
                                ...
                                //判断引入thinkertable
                                if(layui.thinkerTable) {
                                    layui.thinkerTable.saveCols(that.config, dict);
                                }
                            });
                        }
                    });
                ...
                case 'LAYTABLE_EXPORT': //导出
                    if(layui.thinkerTable) {
                        table.exportFile.call(that, options.id, null, null);
                        return;
                    }
                    if(device.ie){
                ...
                case 'LAYTABLE_PRINT': //打印
                    ...
                    break;
                default:
                    if(layui.conf && layui.conf.table.toolbar.events){
                        layui.conf.table.toolbar.events[events] &&
                        layui.conf.table.toolbar.events[events](that, othis);
                    }
            }
        ...
        that.layBody.on('click', 'td', function(e){
            ...
            //如果存在thinkerTable，覆盖渲染
            if(layui.thinkerTable) {
                if($(e.target).attr("class").indexOf("layui-table-edit") == -1) {
                    if(layui.thinkerTable.render(that, othis, commonMember)) {
                        layui.stope(e);
                        return;
                    }
                } else {
    ...
    table.exportFile = function(id, data, type){
        ...
        if(layui.thinkerTable) {
            layui.thinkerTable.exportXlsx(that.config);
            return ;
        }
